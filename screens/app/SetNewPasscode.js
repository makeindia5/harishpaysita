...
const SetNewPasscode = () => {
  ...
  const savePasscode = async (finalPasscode) => {
  try {
    const storedUser = await AsyncStorage.getItem("user");
    const user = JSON.parse(storedUser);

    await axios.post(
      "https://freepe.in/api/security/set-passcode",
      {
        userId: user.id,
    ...
    Alert.alert(
      t("passcode_success", "Passcode saved successfully"),
      "",
      [{ text: "OK", onPress: () => navigation.navigate("Security") }]
    );

  } catch (err) {
    console.log("save passcode error:", err);
    Alert.alert("Error", "Unable to save passcode");
  }
};
    ...
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.iconWrapper}>
          <Ionicons name="key" size={64} color={colors.success} />
        </View>

        {/* Title & Subtitle */}
        <Text style={styles.title}>
          {t('set_new_passcode', 'Create Your Passcode')}
        </Text>
